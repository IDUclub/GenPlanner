...
class GenPlannerFuncZonesDTO(GenPlannerDTO):
    ...
    def validate_project_id(cls, v):
        if v.isnumeric():
            return int(v)
        raise http_exception(400, "Project ID is invalid", _input=v, _detail="Input should be numeric")


class GenPlannerTerZonesDTO(GenPlannerDTO):
    project_id: int = Field(..., description="Project ID")
    scenario: Literal[1, 4, 7, 2, 6, 5, 3] | int = Field(..., description="Scenario ter zone type")
