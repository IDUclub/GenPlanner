...
class GenPlanner:
    ...
    def poly2block(self, terr_zone: TerritoryZone) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    ...
    def poly2terr(self, funczone: FuncZone = basic_func_zone) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
        if self.source_multipolygon:
            return self._run(
    ...
    def poly2terr2block(self, funczone: FuncZone = basic_func_zone) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
        if self.source_multipolygon:
            return self._run(
    ...
    def poly2func(self, genplan: GenPlan = gen_plan) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
        if self.source_multipolygon:
            raise NotImplementedError("Multipolygon source is not supported yet")
        return self._run(poly2func2terr2block_initial, self.transformed_poly, genplan, False, local_crs=self.local_crs)

    def poly2func2terr2block(self, genplan: GenPlan = gen_plan) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
        if self.source_multipolygon:
            raise NotImplementedError("Multipolygon source is not supported yet")
        return self._run(poly2func2terr2block_initial, self.transformed_poly, genplan, True, local_crs=self.local_crs)


def parallel_split_queue(task_queue: multiprocessing.Queue, local_crs) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    splitted = []
    roads_all = []
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        while True:
            ...
            if not future_to_task and task_queue.empty():
                break
    roads_all = [roads_df for roads_df in roads_all if not (roads_df.is_empty())]
    if len(roads_all) > 0:
