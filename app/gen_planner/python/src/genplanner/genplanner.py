...
class GenPlanner:
    ...
    def features2blocks(self, terr_zone: TerritoryZone) -> (gpd.GeoDataFrame, gpd.GeoDataFrame):
        ...
        return self._run(gdf2block_initial, self.territory_to_work_with)

    def features2terr_zones(self, funczone: FuncZone = basic_func_zone) -> (gpd.GeoDataFrame, gpd.GeoDataFrame):
        args = self.territory_to_work_with, funczone, False
        if self.source_multipolygon:
            return self._run(features2terr2block_initial, *args)
        return self._run(feature2terr2block_initial, *args)

    def features2terr_zones2block(self, funczone: FuncZone = basic_func_zone) -> (gpd.GeoDataFrame, gpd.GeoDataFrame):
        args = self.territory_to_work_with, funczone, True
        if self.source_multipolygon:
            return self._run(features2terr2block_initial, *args)
        return self._run(feature2terr2block_initial, *args)
