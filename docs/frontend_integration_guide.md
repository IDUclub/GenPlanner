...
| `error` | LLM-шаг упал целиком (напр. Ollama недоступна) | `stage`, `detail` — в этом случае `token`/`result` не придут вообще, сразу `done` |
...
- **503**, если чат не сконфигурирован на бэкенде (`OLLAMA_BASE_URL`/`CHAT_MODEL` не
