...
| `warning` | сбой истории чата (ChatStorage недоступен) — **не фатально**, ответ всё равно придёт | `stage`, `detail`, `message` (готовый текст для пользователя) |
...
- **`warning` — не то же самое, что `error`.** История чата (ChatStorage) может быть
  недоступна — ход всё равно доработает и пришлёт `token`/`result` как обычно, просто без
  сохранения. Показывайте `warning.message` как ненавязчивый тост, не блокируйте UI.
